...
def create_hourglass_network(num_classes, num_stacks, num_channels, inres, outres):
    input = Input(shape=(inres[0], inres[1], 3))

    front_features = create_front_module(input, num_channels)
...
def hourglass_module(bottom, num_classes, num_channels, hgid):
    left_features = create_left_half_blocks(bottom, hgid, num_channels)

    rf1 = create_right_half_blocks(left_features, hgid, num_channels)
...
def create_front_module(input, num_channels):
    ...
    _x = bottleneck_block(_x, int(num_channels / 2), 'front_residual_x1')
    _x = MaxPool2D(pool_size=(2, 2), strides=(2, 2))(_x)

    _x = bottleneck_block(_x, int(num_channels / 2), 'front_residual_x2')
    _x = bottleneck_block(_x, num_channels, 'front_residual_x3')

    return _x


def create_left_half_blocks(bottom, hglayer, num_channels):
    # create left half blocks for hourglass module
    # f1, f2, f4 , f8 : 1, 1/2, 1/4 1/8 resolution

    hgname = 'hg' + str(hglayer)

    f1 = bottleneck_block(bottom, num_channels, hgname + '_l1')
    _x = MaxPool2D(pool_size=(2, 2), strides=(2, 2))(f1)

    f2 = bottleneck_block(_x, num_channels, hgname + '_l2')
    _x = MaxPool2D(pool_size=(2, 2), strides=(2, 2))(f2)

    f4 = bottleneck_block(_x, num_channels, hgname + '_l4')
    _x = MaxPool2D(pool_size=(2, 2), strides=(2, 2))(f4)

    f8 = bottleneck_block(_x, num_channels, hgname + '_l8')

    return (f1, f2, f4, f8)


def connect_left_to_right(left, right, name, num_channels):
    '''
    ...
    _xleft = bottleneck_block(left, num_channels, name + '_connect')
    _xright = UpSampling2D()(right)
    add = Add()([_xleft, _xright])
    out = bottleneck_block(add, num_channels, name + '_connect_conv')
    return out
...
def bottom_layer(lf8, hgid, num_channels):
    ...
    lf8_connect = bottleneck_block(lf8, num_channels, str(hgid) + "_lf8")

    _x = bottleneck_block(lf8, num_channels, str(hgid) + "_lf8_x1")
    _x = bottleneck_block(_x, num_channels, str(hgid) + "_lf8_x2")
    _x = bottleneck_block(_x, num_channels, str(hgid) + "_lf8_x3")
...
def create_right_half_blocks(leftfeatures, hglayer, num_channels):
    lf1, lf2, lf4, lf8 = leftfeatures

    rf8 = bottom_layer(lf8, hglayer, num_channels)

    rf4 = connect_left_to_right(lf4, rf8, 'hg' + str(hglayer) + '_rf4', num_channels)

    rf2 = connect_left_to_right(lf2, rf4, 'hg' + str(hglayer) + '_rf2', num_channels)

    rf1 = connect_left_to_right(lf1, rf2, 'hg' + str(hglayer) + '_rf1', num_channels)
